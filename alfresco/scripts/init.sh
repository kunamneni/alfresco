...
TOMCAT_CSRF_ENABLED=${TOMCAT_CSRF_ENABLED:-false}

S3_PASSWORD_FILE=${S3_PASSWORD_FILE:-~/.passwd-s3fs}
S3_CONTENT_BUCKET=${S3_CONTENT_BUCKET:-alfresco-s3-content}
S3_DATA_BUCKET=${S3_DATA_BUCKET:-alfresco-s3-data}
...
function mount_s3bucket {
  echo "Mounting S3 folder"
  # Set S3 credentials
  echo  ${S3_ACCESS_KEY}:${S3_SECRET} > $S3_PASSWORD_FILE
  chmod 600 $S3_PASSWORD_FILE
  echo "Mounting $CONTENT_STORE"
  s3fs $S3_CONTENT_BUCKET $CONTENT_STORE -o passwd_file=$S3_PASSWORD_FILE -o nonempty
  echo "Mounting /alfresco/alf_data"
  s3fs $S3_DATA_BUCKET /alfresco/alf_data -o passwd_file=$S3_PASSWORD_FILE -o nonempty
  sleep 5
}
...
if [ "$TOMCAT_CSRF_ENABLED" == "false" ] && [ -f "$TOMCAT_CSRF_PATCH" ] ;then
  patch -Np0 $ALF_HOME/tomcat/shared/classes/alfresco/web-extension/share-config-custom.xml $TOMCAT_CSRF_PATCH
fi
